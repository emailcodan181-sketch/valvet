...
function MetricCard({ label, value, sub, color }) {
  return (
    <div style={{
      ...
      {sub && (
        <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: 4 }}>{sub}</div>
      )}
    </div>
  )
}
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
        <div style={{ fontFamily: 'Share Tech Mono', color: 'var(--neon-cyan)', fontSize: '14px', marginBottom: 16 }}>
          ✓ CÓDIGO GERADO
        </div>
        <div style={{
          padding: '12px 16px',
          ...
          color: 'var(--neon-cyan)', wordBreak: 'break-all',
          marginBottom: 12,
        }}>
          {code.code}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={onClose} style={{
            ...
            border: '1px solid rgba(100,116,139,0.3)', color: 'var(--text-muted)',
...
function LogsPanel({ authFetch }) {
  ...
  const exportCsv = () => {
    ...
    const rows   = logs.map((l) =>
      `"${l.createdAt}","${l.username}","${l.ip}","${l.status}","${l.reason || ''}","${(l.userAgent || '').substring(0, 60)}"`
    ).join('\n')
      ...
      <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(157,78,221,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <select
            ...
            style={{ padding: '5px 10px', fontSize: '11px', fontFamily: 'JetBrains Mono', background: 'var(--bg-tertiary)', border: '1px solid rgba(157,78,221,0.25)', color: 'var(--text-muted)' }}
          >
          ...
          <input
            ...
            style={{ padding: '5px 10px', fontSize: '11px', width: 180 }}
          />
          ...
          <button onClick={exportCsv} style={{ padding: '5px 12px', background: 'transparent', border: '1px solid rgba(157,78,221,0.3)', color: 'var(--neon-purple)', fontSize: '11px', fontFamily: 'JetBrains Mono' }}>
            Exportar CSV
          </button>
        </div>
      </div>
            ...
            <tbody>
              {logs.length === 0 ? (
                ...
                <tr key={l.id} className={l.status === 'FAILURE' ? 'row-failure' : ''}>
                  ...
                  <td style={{ fontSize: '11px', color: 'var(--text-muted)', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {(l.userAgent || '').substring(0, 60)}
                  </td>
                </tr>
              ))}
            </tbody>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingBottom: 40 }}>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <span style={{ fontFamily: 'Share Tech Mono', fontSize: '16px', color: 'var(--neon-purple)', letterSpacing: '0.1em' }}>
            VALVET
          </span>
          <span style={{ fontSize: '11px', color: 'var(--neon-pink)', letterSpacing: '0.1em' }}>
            SYS / CONTROL-PANEL
          </span>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{user?.username}</span>
          <button onClick={() => navigate('/')} style={{ background: 'transparent', border: '1px solid rgba(157,78,221,0.3)', color: 'var(--neon-purple)', fontFamily: 'JetBrains Mono', fontSize: '11px', padding: '5px 12px' }}>
            Chat
          </button>
          <button onClick={handleLogout} style={{ background: 'transparent', border: '1px solid rgba(100,116,139,0.3)', color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', fontSize: '11px', padding: '5px 12px' }}>
            Logout
          </button>
        </div>
      </div>
